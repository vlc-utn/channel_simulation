%% Time Response
% Simulate temporal response of channel for a single point. This code is 
% based on the "rms_spread" code.
clc; clear; close all;
...
title("LOS + NLOS temporal response");
xlabel("time [ns]");
ylabel("h(t)");

figure();
plot(t_vector/1e-9, h_vector_los);
title("LOS temporal response");
xlabel("time [ns]");
ylabel("h(t)");

figure();
plot(t_vector/1e-9, h_vector_nlos);
title("NLOS temporal response");
xlabel("time [ns]");
ylabel("h(t)");
